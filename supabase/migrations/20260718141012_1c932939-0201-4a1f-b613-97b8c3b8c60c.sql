...
RETURNS TABLE(
  id uuid, name text, brand text, quantity numeric, unit text,
  image_url text, barcode text, category_id uuid, category_name text,
  category_slug text, min_price numeric, reference_price numeric,
  max_discount_pct numeric, market_count integer,
  last_updated timestamp with time zone, has_promotion boolean
)
...
AS $function$
  WITH tokens AS (
    SELECT array_remove(
      string_to_array(
        regexp_replace(trim(public.immutable_unaccent(COALESCE(q,''))), '\s+', ' ', 'g'),
        ' '
      ),
      ''
    ) AS toks
  ),
  -- Linked path: aggregated by canonical product
  linked_prices AS (
    SELECT sp.canonical_product_id, sp.store_id, cp.effective_price,
           cp.regular_price, cp.promotional_price, cp.collected_at, sp.available
  ...
  linked_rows AS (
    SELECT p.id, p.name, p.brand, p.quantity, p.unit, p.image_url, p.barcode,
    ...
    JOIN linked_agg a ON a.canonical_product_id = p.id
  ...
  -- Unlinked path: one row per store_product
  unlinked_rows AS (
    SELECT sp.id AS id,
           ...
           cp.effective_price AS min_price,
           cp.regular_price AS reference_price,
           (CASE WHEN cp.promotional_price IS NOT NULL AND cp.regular_price > 0
                 THEN ((cp.regular_price - cp.promotional_price)/cp.regular_price)*100
                 ELSE 0 END)::numeric AS max_discount_pct,
           1 AS market_count,
           cp.collected_at AS last_updated,
           (cp.promotional_price IS NOT NULL) AS has_promotion
    FROM public.store_products sp
    JOIN public.current_prices cp ON cp.store_product_id = sp.id
    WHERE sp.active = true
        ...
        NOT EXISTS (
          ...
          WHERE tok <> ''
            AND public.immutable_unaccent(
                  ...
                  COALESCE(sp.brand_raw,'') || ' ' ||
                  COALESCE(sp.quantity_raw,'') || ' ' ||
                  COALESCE(sp.unit_raw,'')
                ) NOT ILIKE '%' || tok || '%'
        )
      )
  ),
