-- A diversificação por tipo (migração anterior) passou a exibir também
-- produtos SEM promoção — mas a seção da tela inicial se chama "Ofertas
-- por categoria", então só deveria trazer produto que está realmente em
-- oferta em algum mercado agora. Adiciona esse filtro de volta, mantendo
-- o rodízio por tipo (arroz, feijão, café…) só entre os que estão em
-- promoção.
...
AS $function$
  ...
  marked AS (
    ...
    FROM grouped g
    -- Tela inicial é "Ofertas por categoria" — só entra quem está
    -- realmente em promoção em algum mercado agora.
  ...
  bucket_ranked AS (
    SELECT
      ...
      ROW_NUMBER() OVER (
        ...
        ORDER BY (m.market_count > 1) DESC, m.max_discount_pct DESC, random()
      ) AS bucket_rank
    FROM marked m
  ),
  final_ranked AS (
