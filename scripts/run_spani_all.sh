...
for variable in INGEST_URL INGEST_SECRET SPANI_STORE_ID; do
  if [[ -z "${!variable:-}" ]]; then
...
LOCK_DIR=".spani_collection.lock"
...
SUMMARY_FILE="logs/resumo_spani_${RUN_TIMESTAMP}.log"
...
for category in "${CATEGORIES[@]}"; do
  LOG_FILE="logs/spani_${category}_${RUN_TIMESTAMP}.log"

  echo ""
  echo "Iniciando categoria: $category"
  echo "Log: $LOG_FILE"

  "$PYTHON_BIN" scripts/collectors/spani_marilia.py \
    --categoria "$category" \
    --paginas "$PAGES" \
    2>&1 | tee "$LOG_FILE"
