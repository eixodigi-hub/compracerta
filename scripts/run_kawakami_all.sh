...
for variable in INGEST_URL INGEST_SECRET KAWAKAMI_STORE_ID; do
  if [[ -z "${!variable:-}" ]]; then
...
LOCK_DIR=".kawakami_collection.lock"
...
# do Kawakami (perfumaria_higiene, limpeza) passam de 100 páginas.
PAGES="${KAWAKAMI_PAGES:-250}"
CATEGORY_DELAY="${CATEGORY_DELAY:-8}"

START_TIME="$(date '+%Y-%m-%d %H:%M:%S')"
RUN_TIMESTAMP="$(date '+%Y%m%d_%H%M%S')"
SUMMARY_FILE="logs/resumo_kawakami_${RUN_TIMESTAMP}.log"
...
for category in "${CATEGORIES[@]}"; do
  LOG_FILE="logs/kawakami_${category}_${RUN_TIMESTAMP}.log"

  echo ""
  echo "Iniciando categoria: $category"
  echo "Log: $LOG_FILE"

  "$PYTHON_BIN" scripts/collectors/kawakami_marilia.py \
    --categoria "$category" \
    --paginas "$PAGES" \
    2>&1 | tee "$LOG_FILE"
