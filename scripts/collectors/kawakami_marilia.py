"""
Coletor externo para Supermercados Kawakami — loja de Marília/SP.

O site (kawakami.com.br) é uma SPA sobre a plataforma VipCommerce — a
mesma usada pelo Spani; não expõe HTML com os produtos, mas usa uma
API JSON pública em services.vipcommerce.com.br. Este coletor fala
diretamente com essa API (sem navegador).

Uso rápido:

    export INGEST_SECRET="<segredo>"
    export KAWAKAMI_STORE_ID="<uuid do mercado Kawakami Marília>"
    python3 scripts/collectors/kawakami_marilia.py --dry-run --categoria hortifruti --paginas 2
...
- Faz login com a chave pública do app da loja (o mesmo valor que
  ...
  segredo de conta, é a credencial "app" do storefront, compartilhada
  por toda a plataforma VipCommerce — o mesmo valor já usado pelo
  coletor do Spani funciona aqui também).
- Coleta apenas o catálogo da loja de Marília (centro de distribuição
  1 — "Loja Marilia" / "Marilia Zona Sul", Av. João Ramalho 2270),
  identificada por inspeção da própria API pública.
...
BASE_SITE_URL = "https://www.kawakami.com.br"
API_BASE_URL = "https://services.vipcommerce.com.br/api-admin/v1"
IMAGE_BASE_URL = (
    "https://produto-assets-vipcommerce-com-br.br-se1.magaluobjects.com/250x250"
)

# Identificadores da loja Kawakami Marília no VipCommerce, descobertos
# por inspeção da API pública em 2026-09 (GET .../loja/centros_distribuicoes/retiradas
# retorna "Loja Marilia" / "Marilia Zona Sul", Av. João Ramalho 2270,
# id interno 1 — não confundir com o campo
# "vipcommerce_centro_distribuicao_id"=146 da mesma resposta, que não é
# o valor usado nas rotas de produtos). "LOGIN_KEY" é a chave pública
# do app do storefront, igual em toda a plataforma VipCommerce.
DOMAIN_KEY = "kawakami.com.br"
ORGANIZATION_ID = "100"
FILIAL_ID = "1"
...
MAX_RETRIES = 3
BATCH_SIZE = 50
# Piso de segurança: mesmo raciocínio do coletor do Spani — se algum
# dia passar disso é sinal de algo errado (loop infinito, paginação
# quebrada), não catálogo legítimo.
HARD_PAGE_LIMIT = 150
TZ_SP = ZoneInfo("America/Sao_Paulo")

# classificacao_mercadologica_id (departamento) -> chave interna. Usa
# as MESMAS chaves internas do coletor do Spani onde o departamento
# corresponde ao mesmo conceito, pra reaproveitar o mapeamento de
# categoria do site (src/lib/source-category-map.ts) sem precisar
# adicionar entradas novas. Lista obtida em
...
log = logging.getLogger("kawakami")
...
def normalize_product(raw: dict, category_key: str) -> Produto | None:
    ...
    return Produto(
        ...
        promotion_text=("Oferta Kawakami" if promotional is not None else None),
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Coletor Kawakami Marília")
    p.add_argument(
...
def main() -> int:
    ...
    store_id = os.environ.get("KAWAKAMI_STORE_ID", "")
    secret = os.environ.get("INGEST_SECRET", "")
    delay = env_float("REQUEST_DELAY_SECONDS", 1.0)

    if not args.dry_run and (not store_id or not secret):
        log.error(
            "Defina KAWAKAMI_STORE_ID e INGEST_SECRET no ambiente (ou use --dry-run)."
        )
        return 2
